...
{
	$cname= mysqli_real_escape_string($con,$_POST['company_name']);
	$name = mysqli_real_escape_string($con,$_POST['name']);
	$udesc = mysqli_real_escape_string($con,$_POST['description']);
	$status = mysqli_real_escape_string($con,$_POST['status']);
	
	
	$sql = "INSERT INTO location(company_id,name,description,status) VALUES('$cname','$name','$udesc','$status')";
    ...
    ?>
          ...
          <input type="radio" name="status" value="0"> Non_Active<br>
            ...
            <div class=" col-sm-8 btn-right">
         	 <button type="submit"  name="btn-default"class="btn btn-default" >Add Location here</button>
            </div>
            </div>
            </div>
            </form>
